  ...
  <div>
      ...
      <div class="allcontent" v-if="show">
        <ul class="hotShangping" style="overflow-y:scroll;height:65vh">
          <li :key="i" v-for="(value,i) in food" class="lis1">
            <a :href="'#a'+ value.id" class="da">{{value.name}}</a>
          ...
          <div>
            <ul>
              <li :key="i3" v-for="(v,i3) in value.foods" class="man">
                ...
                <p v-if="v.specifications[0]" class="gege">
                  <router-link
                    :to="'/guige?cityId=' + v.specifications[0].values[1]+'&yi='+v.specifications[0].values[0]+'&hao='+v.name+'&qibu='+v.specfoods[0].price"
                  >
                    <span>{{v.specifications[0].name}}</span>
                  </router-link>
                </p>
                
                <p v-if="!v.specifications[0]" class="gege" @click="tianjia(v.specfoods[0].price,v.name,i2,i3)">
                +
                </p>
                <span class="snow" v-if="v.is_featured>0">{{v.is_featured}}</span>
                 <p v-if="v.is_featured>0" class="s" @click="jian(v.specfoods[0].price,v.name,i2,i3)">
                -
                </p>
                <div id="yang"></div>
   ...
   <div id="alldian" v-if="jia">
    ...
    <div id="jiesuan">
      ...
      <p class="yi" @click="alldian()">￥{{this.$store.state.zongjia}}</p>
    ...
    <router-view></router-view>
     <div id="ball"></div>
     <div id="push" @click.stop="fanjia()"><img src="../assets/img/back.png"></div>
  </div>
</template>
<script>
export default {
